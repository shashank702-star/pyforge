...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tabStdout = document.getElementById('terminal-tab-stdout');
  const tabNetwork = document.getElementById('terminal-tab-network');
  ...
  function logNetworkEvent(url, status, reason) {
    if (!networkLogBody) return;
    
    // Clear empty state on first log
    const emptyState = networkLogBody.querySelector('.network-empty-state');
    if (emptyState) {
      networkLogBody.innerHTML = '';
    }
    
    // Create network log item element
    const logItem = document.createElement('div');
    logItem.className = 'network-log-item';
    
    const timestamp = new Date().toLocaleTimeString();
    const badgeClass = status.toLowerCase() === 'allowed' ? 'allowed' : 'blocked';
    
    const headerDiv = document.createElement('div');
    headerDiv.className = 'network-log-header';
    
    const urlSpan = document.createElement('span');
    urlSpan.className = 'network-log-url';
    urlSpan.textContent = url;
    
    const badgeSpan = document.createElement('span');
    badgeSpan.className = `net-badge ${badgeClass}`;
    ...
    networkLogBody.appendChild(logItem);
    networkLogBody.scrollTop = networkLogBody.scrollHeight;
  }

  // Intercept window.fetch to block malicious tracking calls
  ...
  window.fetch = async function(input, init) {
    ...
    const isBlocked = domainBlacklist.some(domain => {
      return hostname.includes(domain) || url.toLowerCase().includes(domain);
    });
    
    if (isBlocked) {
      logNetworkEvent(url, 'BLOCKED', 'Telemetry or tracking endpoint matched.');
      logTerminal(`\n[SECURITY WARNING] Blocked outbound connection request to: ${url}\nReason: Malicious tracker/telemetry endpoint matched.`, 'error');
      throw new TypeError("Network Request Blocked: Endpoint matches malicious domain blacklist.");
    } else {
      logNetworkEvent(url, 'ALLOWED', 'Legitimate outbound request passed audit.');
      return originalFetch.apply(this, arguments);
    }
  };

  if (tabStdout && tabNetwork) {
    tabStdout.addEventListener('click', () => {
      tabStdout.classList.add('active');
      tabNetwork.classList.remove('active');
      terminalBody.classList.remove('hidden');
      if (networkLogBody) networkLogBody.classList.add('hidden');
    });
    
    tabNetwork.addEventListener('click', () => {
      tabNetwork.classList.add('active');
      tabStdout.classList.remove('active');
      terminalBody.classList.add('hidden');
      if (networkLogBody) networkLogBody.classList.remove('hidden');
    });
  }
  ...
  function clearTerminal() {
    ...
    if (networkLogBody && !networkLogBody.classList.contains('hidden')) {
      networkLogBody.innerHTML = `
        <div class="network-empty-state" style="text-align: center; padding: 30px; color: var(--text-muted); font-size: 0.9rem; margin: auto;">
          <i class="fa-solid fa-shield-halved text-mint" style="font-size: 1.8rem; display: block; margin-bottom: 12px;"></i>
          Network Intrusion Tracker Active. Running outbound audits...
        </div>
      `;
    }
  }
...
sys.stderr = io.StringIO()
  ...
  function executePythonCodeFallback(code) {
    ...
    if (val !== 'custom') {
      ...
      } else if (val === 'security-attack') {
        mockStdout = "Initiating outbound telemetry synchronization...\nNetwork Connection Failed: TypeError: Network Request Blocked: Endpoint matches malicious domain blacklist.";
        logNetworkEvent("https://doubleclick.net/telemetry/sync?user=Archimedes&xp=100", 'BLOCKED', 'Telemetry or tracking endpoint matched.');
      }
